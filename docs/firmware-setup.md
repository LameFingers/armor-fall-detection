...
1. The **sender** OLED should show `SENDER / MONITORING` with a live
   acceleration reading near **1.0 g** at rest.
...
3. Move the sender board quickly — if acceleration exceeds `MOTION_THRESHOLD_G`
   (1.2 g default) the sender LED turns red, the buzzer sounds, and a
   `FALL_ALERT` packet is transmitted.
4. The **receiver** should respond with a red LED, buzzer, and OLED alert
   showing the RSSI and SNR values of the received packet.
5. Both boards return to their normal states after approximately two seconds.
